...
function InvoiceList() {
    ...
    return (
        <>
                            ...
                            {invoices.map((item, index) => (
                                <div className="invoice__item" key={index}>
                                    {item.invoice_image_base64 &&
                                        <Base64Image base64String={item.invoice_image_base64} alt={`Selected file ${index + 1}`} />
                                    }
            ...
            {selectedInvoiceId && (
                <InvoiceInfo invoiceId={selectedInvoiceId} onClose={handleCloseInvoice} onInvoiceDeleted={handleInvoiceDeleted} />
            )}
        </>
    );
}
